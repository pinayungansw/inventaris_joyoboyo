...
<div class="space-y-6">
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <form class="flex items-center gap-3" method="GET">
            <select name="lokasi_id" onchange="this.form.submit()" class="form-control">
                <option value="">Semua Lokasi</option>
                @foreach($lokasi as $l)
                <option value="{{ $l->id }}" {{ request('lokasi_id')==$l->id ? 'selected' : '' }}>{{ $l->nama_lokasi }}
                </option>
                @endforeach
            </select>
        </form>
            ...
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    <tr class="hover:bg-slate-50/50">
                        ...
                        <td class="px-6 py-3 font-medium text-slate-700">{{ $r->nama_ruangan }}</td>
                        <td class="px-6 py-3 text-slate-600">{{ $r->lokasi->nama_lokasi }}</td>
                        ...
                        <td class="px-6 py-3 text-center">
                            <div class="flex items-center justify-center gap-2">
                                <a href="{{ route('master.ruangan.edit', $r) }}"
                                    class="rounded-lg bg-amber-50 p-2 text-amber-600 hover:bg-amber-100 transition">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                    </svg>
                                </a>
                                <form action="{{ route('master.ruangan.destroy', $r) }}" method="POST"
                                    onsubmit="return confirm('Yakin?')">
                                    @csrf @method('DELETE')
                                    <button class="rounded-lg bg-red-50 p-2 text-red-600 hover:bg-red-100 transition">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    @empty
                    <tr>
                        <td colspan="6" class="px-6 py-8 text-center text-slate-400">Belum ada data ruangan.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
